    ...
    if facebook_pages.empty?
      ...
      if facebook_pages_associated_to_instagram_business_accounts.present?
        ...
        if user_token_instagram_basic_permission
          ...
          if existing_business_account && existing_business_account[:instagram].consumer.facebook
            existing_business_account[:instagram].consumer.facebook.update(uid: request.env["omniauth.auth"]["uid"])
          end
          facebook = Identity.find_or_initialize_for_oauth(request.env["omniauth.auth"])
          flash.notice = "Welcome !"
          if existing_business_account
            consumer = existing_business_account[:instagram].consumer
            facebook_page = existing_business_account[:facebook_page]
            instagram = existing_business_account[:instagram]
            facebook_page.update(consumer_id: consumer.id)
            instagram.update(consumer_id: consumer.id)
            facebook.update(consumer_id: consumer.id)
            sign_in_and_redirect(consumer, event: :authentication)
          else
            consumer = Consumer.new(email: "instagram_#{business_accounts.first[:instagram].uid}@footemp.com")
